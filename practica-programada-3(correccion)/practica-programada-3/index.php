...
<body>
    ...
    <?php
    // Verificar si el archivo txt donde se almacena la informacion existe
    if (file_exists('transacciones.txt')) {
        $transacciones = unserialize(file_get_contents('transacciones.txt'));
    } else {
        file_put_contents('transacciones.txt', serialize($transacciones));
    }
    ...
    if (isset($_POST['monto']) && isset($_POST['descripcion'])) {
        ...
        // Guardar información en el arreglo
        $transacciones[] = $nueva_transaccion;
        file_put_contents('transacciones.txt', serialize($transacciones));

    // Eliminar las transacciones a partir de su id
    } elseif (isset($_POST['eliminar_id'])) {
        $eliminar_id = $_POST['eliminar_id'];
        foreach ($transacciones as $idTrans => $transaccion) {
            if ($transaccion['id'] == $eliminar_id) {
                unset($transacciones[$idTrans]);
                break;
            }
        }
    ...
    function generarEstadoDeCuenta($transacciones) {
        ...
        echo "</tbody>";
        echo "</table>";
        echo "<div class='mt-3 p-3 text-center border'>";
    ...
    // Información que se agrega en el txt
